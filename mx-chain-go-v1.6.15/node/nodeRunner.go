...
//? ----------------------- UNUSED -----------------------
func (h *AccountMigrationHandler) EpochConfirmedOLD(epoch uint32, timestamp uint64) {
...
func (h *AccountMigrationHandler) EpochConfirmed(epoch uint32, timestamp uint64) {
	...
	accountsAllocation := h.currentNode.processComponents.EpochStartTrigger().GetAccountAllocationFromCurrentEpochStartBlock()

	if len(accountsAllocation) == 0{
		log.Debug("*** NO ACCOUNTS MIGRATION TO BE PERFORMED ***", "epoch", epoch, "timestamp", timestamp)
		return
	}


	log.Debug("*** ACCOUNT ALLOCATION IS NOT EMPTY: PERFORMING ACCOUNT MIGRATIONS ***", "numMigrations", len(accountsAllocation))

	for _, accountMigrationHandler := range accountsAllocation{
		currentAccountsMapping := h.currentNode.processComponents.ShardCoordinator().UpdateAccountsMappingEntryFromAddressString(
			accountMigrationHandler.GetAccountAddressString(),
			accountMigrationHandler.GetDestinationShard(),
			epoch,
		)		
		log.Debug("***Current Accounts Mapping***", "accountsMapping", currentAccountsMapping)
		rootHashBeforeNewAccount, err := h.currentNode.stateComponents.AccountsAdapter().RootHash()
		...
		createSingleAccountMigrationTransaction(
			h,
			epoch,
			accountMigrationHandler.GetAccountAddressString(),
			accountMigrationHandler.GetSourceShard(),
			accountMigrationHandler.GetDestinationShard(),
			accountMigrationHandler.GetMigrationNonce(),
		)
	}

	//? clean accountAllocationFromCurrentEpochStartBlock once processed
	h.currentNode.processComponents.EpochStartTrigger().CleanAccountAllocationFromCurrentEpochStartBlock()
}
...
func createSingleAccountMigrationTransaction(h *AccountMigrationHandler, epoch uint32, accountAddressToBeMigrated string, sourceShard uint32, destShard uint32, migrationNonce uint64){
