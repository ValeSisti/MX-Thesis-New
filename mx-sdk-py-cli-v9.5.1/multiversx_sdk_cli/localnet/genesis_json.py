...
def build(config: ConfigRoot) -> List[Any]:
    ...
    for nickname, account in wallets.get_users().items():
        ...
        value = remaining_supply if is_last_user(nickname) else 1000000000000000000  #original_value = 100000000000000000000000
        delegated_value = remaining_to_delegate if is_last_user(nickname) else 1000000000000000 #original_value = 100000000000000000000
